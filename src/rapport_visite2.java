...
public class rapport_visite2 extends accueil {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
				    ...
				    result = state.executeQuery("SELECT RAP_NUM as numRapport,SUBSTRING(RAP_DATE,1,10) as dateRapport from rapport_visite where VIS_MATRICULE<>'"+ matricule + "' "
				    		+ "and VIS_MATRICULE in (SELECT VIS_MATRICULE from travailler where REG_CODE in ("
			...
			prec.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					try {
						...
						result2=state.executeQuery("SELECT * from rapport_visite where VIS_MATRICULE<>'"+ matricule + "' "
				    		+ "and VIS_MATRICULE in (SELECT VIS_MATRICULE from travailler where REG_CODE in ("
